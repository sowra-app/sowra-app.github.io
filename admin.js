...
async function loadEC(){
  ...
  try{
    ...
    if(noms.length){
      try{
        ...
        (v.data||[]).forEach(x=>{
          ...
          if(USER&&x.voter===USER.id)myVotes[x.nom_id]=x.vote;
        });
      }catch(e){}
    }
    ...
    el.innerHTML=head+noms.map(n=>{
      const p=admPhotos.find(x=>x.id===n.photo_id)||photos.find(x=>x.id===n.photo_id);
...
async function ecNominate(pid){
  ...
  try{
    ...
    if(nn)await sb.from('ec_votes').insert({nom_id:nn.id,voter:USER.id,vote:'yes'});
  }catch(e){}
...
async function ecVote(nomId,vote){
  ...
  const {error}=await sb.from('ec_votes').upsert(
    {nom_id:nomId,voter:USER.id,vote},
...
async function admNewsSave(){
  ...
  if(window.__SPDATA){window.__SPDATA.news_title=t;window.__SPDATA.news_body=bd;window.__SPDATA.news_id=nid}
  if(window.__SPB){window.__SPB.news_title=t;window.__SPB.news_body=bd;window.__SPB.news_id=nid}
...
async function admNewsToggle(){
  ...
  if(error){toast('تعذرت العملية: '+error.message,true);return}
  if(window.__SPDATA)window.__SPDATA.news_on=nv;
  if(window.__SPB)window.__SPB.news_on=nv;
